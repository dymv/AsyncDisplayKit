...
- (void)nodeDidAppear:(ASDisplayNode *)node
{
  ...
  if (!node.layerBacked && node.view.window != self.appVisibleWindow) {
    return;
  }
...
- (void)runLoopDidTick
{
  ...
  self.tipWindow.nodeToTipStates = [_nodeToTipStates copy];
...
- (void)createTipWindowIfNeededWithFrame:(CGRect)tipWindowFrame
{
  ...
  if (self.tipWindow == nil) {
    ...
    // This is just a BS VC so that UIKit doesn't complain
    self.tipWindow.rootViewController = [[UIViewController alloc] init];
    self.tipWindow.windowLevel = UIWindowLevelNormal + 1;
    self.tipWindow.opaque = NO;
